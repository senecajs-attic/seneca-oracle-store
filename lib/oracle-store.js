    ...
    list: function(args,cb) {
      ...
      dbinst.execute(statement.query, statement.values, function( err, results ) {
        if( !error(args, err, cb) ) {
          ...
          var rows = mapOracleResultToRow (results);
          makeentRows(qent, rows, function (err, list) {
              if (err) {
                  return cb(err);
              }
...
var mapOracleResultToRow = function (results) {
    ...
    var columns = results.metaData.map(function (item) {
        return item.name;
    });

    var rows = results.rows.map(function (row) {
        return columns.reduce(function (prev, column, index) {
 ...
 */
var makeentRows = function(ent, rows, callback) {

    async.map(rows, function (row, cb) {
        makeent(ent, row, function (err, res) {
            cb(err, res);
        });
