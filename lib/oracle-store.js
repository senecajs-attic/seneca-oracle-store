  ...
  var store = {
    ...
    save: function(args,cb) {
      ...
      if (update) {
        ...
        dbinst.execute(statement.query, statement.values, function( err, result ) {
          if ( !error(args, err, cb) ) {
            seneca.log(args.tag$, 'save/insert', result, statement.query);
            cb(null, ent);
          }
        })
      }
...
function updateStatement(ent) {
  ...
  query = 'UPDATE ' + tablename(ent) + ' SET ' + setargs.join(', ') + ' WHERE id=:' + i;
...
function selectStatement(qent,q) {
  ...
  query = "SELECT * FROM " + tablename(qent)

  if ( where.args.length ) {
    query += ' WHERE ' + where.args.join(' AND ')
  }
...
function deleteAllStatement(qent,q) {
  ...
  query = "DELETE FROM " + tablename(qent)

  if ( where.args.length ) {
    query = query + ' WHERE ' + where.args.join(' AND ')
  }
